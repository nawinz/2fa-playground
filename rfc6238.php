  ...
  class Base32Static {
   ...
   private static $flippedMap = array(
	...
	class TokenAuth6238 {
      ...
      if(!defined("OTP_OAUTH_LENGTH")){
        die("OTP_OAUTH_LENGTH is not defined");
      }
			...
			for($i=-($rangein30s); $i<=$rangein30s; $i++) {
				...
				if ((int)$code == self::oath_truncate($thiskey,OTP_OAUTH_LENGTH)) {
					return true;
				}
    ...
    public static function verifyNext($secretkey) {
      if(!defined("OTP_OAUTH_LENGTH")){
        die("OTP_OAUTH_LENGTH is not defined");
      }
      $key = base32static::decode($secretkey);
      ...
      for($i=0; $i<=30; $i++) {
        ...
        array_push($b, self::oath_truncate($thiskey,OTP_OAUTH_LENGTH));

      }
      return $b;
    }
    public static function verifyTime($secretkey, $code, $rangein30s = 3) {
      if(!defined("OTP_OAUTH_LENGTH")){
        die("OTP_OAUTH_LENGTH is not defined");
      }
      $key = base32static::decode($secretkey);
    ...
    public static function checktime($secretkey, $code, $rangein30s = 3) {
      if(!defined("OTP_OAUTH_LENGTH")){
        die("OTP_OAUTH_LENGTH is not defined");
      }
      $key = base32static::decode($secretkey);
    ...
    public static function getTokenCodeDebug($secretkey,$rangein30s = 3) {
      ...
      // print "UnixTimeStamp (time()/30): $unixtimestamp <br/>";
        ...
        // print "Calculating oath_hotp from (int)(unixtimestamp +- 30sec offset): $checktime basing on secret key<br/>";
				...
				$thiskey = self::oath_hotp($key, $checktime, false);
        // print "======================================================<br/>";
        // print "CheckTime: $checktime oath_hotp:".$thiskey."<br/>";

        $result = self::oath_truncate($thiskey,OTP_OAUTH_LENGTH,false);
			}

      return $result."|".$checktime;
    }

    public static function getBarCodeUrl($username, $secretkey, $issuer) {
      if(!defined("OTP_OAUTH_LENGTH")){
        die("OTP_OAUTH_LENGTH is not defined");
      }
      $url = "https://chart.apis.google.com/chart";
      $url = $url."?chs=200x200&chld=M|0&cht=qr&chl=otpauth://totp/";
      $url = $url.$username . "%3Fsecret%3D" . $secretkey . '%26issuer%3D' . rawurlencode($issuer).'%26digits%3D'.OTP_OAUTH_LENGTH;

      return $url;
    }
    public static function generateRandom($m){
      $bitMap = str_split("234567QWERTYUIOPASDFGHJKLZXCVBNM");
      return $bitMap[$m];
    }
    public static function generateRandomClueFromMD5($md5){
      if(strlen($md5) != 32){
        $md5 = md5(sha1(md5($md5)));
      }
      $md = str_split(strtoupper($md5));
      $arraymap = array("A","B","C","D","E","F");
      $rou = "";
      $bitMap = str_split("234567QWERTYUIOPASDFGHJKLZXCVBNM");
      foreach ($md as $key => $value) {
          if(is_numeric($value)){
            $rou .= self::generateRandom($value);
          }else{
            $rou .= self::generateRandom(array_flip($arraymap)[$value]);
          }
      }
      $bitMap = str_split("234567QWERTYUIOPASDFGHJKLZXCVBNM");
      $count = "";
      foreach (str_split($rou) as $key => $value) {
        $count .= $bitMap[array_flip($bitMap)[$value]];
      }
      return $count;
    }
